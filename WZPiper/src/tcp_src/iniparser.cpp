...
Config* loadConfig(const char* filePath){
    ...
    if (rtn == INI_SUCCESS ){
        ...
        config->tdAddress = ini.GetStr("Strategy", "TD_SERVER");
        config->mdAddress = ini.GetStr("Strategy", "MD_SERVER");
        config->td_port = atoi(ini.GetStr("Strategy", "TD_PORT"));
        config->md_port = atoi(ini.GetStr("Strategy", "MD_PORT"));
        return config;
    }
    else{
