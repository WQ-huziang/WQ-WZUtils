...
struct WZRspPositionField
{
	...
	WZHedgeFlagType      	HedgeFlag;             //投机套保标志
	WZPosiDirectionType  	PosiDirection;         //持仓多空方向
};
...
struct WZInputOrderField
{
	...
	int                        	Volume;                //数量
	int                        	MinVolume;             //最小成交量
	WZTimeConditionType        	TimeCondition;         //有效期类型
	WZVolumeConditionType      	VolumeCondition;       //成交量类型
	WZOrderPriceTypeType       	OrderPriceType;        //报单价格条件
	WZDirectionType            	Direction;             //买卖方向
	WZOffsetFlagType           	OffsetFlag;            //开平标志
	WZHedgeFlagType            	HedgeFlag;             //投机套保标志
	WZForceCloseReasonType     	ForceCloseReason;      //强平原因
	double                     	StopPrice;             //止损价
	int                        	IsAutoSuspend;         //自动挂起标志
	WZContingentConditionType  	ContingentCondition;   //触发条件
